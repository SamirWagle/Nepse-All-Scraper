...
Company name lookup (tries in order):
    data/companies.csv          — columns: symbol, name  (or ticker, company_name, etc.)
    data/company-wise/{SYMBOL}/info.csv  — any CSV with a "name" column
...
def load_company_names(data_dir: Path) -> dict:
    """
    Build a symbol → company name mapping.

    Tries (in order):
      1. data/companies.csv  with columns: symbol + one of (name, company_name, company)
      2. data/company-wise/{SYMBOL}/info.csv  with a 'name' column
         (only used as a fallback when iterating all symbols)

    Returns a dict keyed by UPPER-CASE symbol. Missing symbols return "".
    """
    mapping = {}

    # ── Attempt 1: root-level companies.csv ───────────────────────────
    for candidate in ["companies.csv", "company.csv", "listed_companies.csv"]:
        ...
        if path.exists():
            try:
                ...
                sym_col = next(
                    (c for c in df.columns if c in ("symbol", "ticker", "stock_symbol")),
                    None
                )
                # Find the name column
                name_col = next(
                    (c for c in df.columns if c in ("name", "company_name", "company", "full_name")),
                    None
                )
    ...
    # ── Attempt 2: per-symbol info.csv (populated lazily on first miss) ─
    # We return the partially-built mapping here; get_company_name() will
    # hit the per-symbol file on demand for any symbol not in the mapping.
    return mapping


_COMPANY_NAMES: dict = {}          # module-level cache
_NAMES_LOADED: bool = False        # whether load_company_names has run
_DATA_DIR_USED: Path = None        # track which data_dir was used


def get_company_name(symbol: str, data_dir: Path) -> str:
    """
    Return the company name for *symbol*, or "" if not found.
    Uses a module-level cache so the CSV is read at most once per run.
    """
    global _COMPANY_NAMES, _NAMES_LOADED, _DATA_DIR_USED

    # Reload if data_dir changed (e.g. different --data-dir flag)
    if not _NAMES_LOADED or _DATA_DIR_USED != data_dir:
        _COMPANY_NAMES = load_company_names(data_dir)
        _NAMES_LOADED = True
        _DATA_DIR_USED = data_dir

    sym = symbol.upper()
    if sym in _COMPANY_NAMES:
        return _COMPANY_NAMES[sym]

    # ── Fallback: per-symbol info.csv ─────────────────────────────────
    info_path = data_dir / "company-wise" / sym / "info.csv"
    if info_path.exists():
        try:
            ...
            if name_col and not df.empty:
                name = str(df[name_col].iloc[0]).strip()
                if name.lower() != "nan":
                    _COMPANY_NAMES[sym] = name   # cache it
                    return name
        except Exception:
            pass

    return ""   # not found — column will be blank rather than crashing
...
def xirr(cash_flows: list) -> float:
    ...
    Uses numpy to solve for rate r where NPV = 0.
    """
    ...
    # Try bracketing between -0.999 and 100 (10000%)
    try:
        from scipy.optimize import brentq
        return brentq(npv, -0.999, 100.0, maxiter=1000)
    except Exception:
        # Fallback: Newton's method via numpy
        rate = 0.1
        for _ in range(1000):
            f  = npv(rate)
...
def calculate_cagr(
    ...
    Returns dict with keys: symbol, company_name, start_date, end_date, years,
    ...
    try:
        ...
        # XIRR cash flows: (date, amount) — outflows negative, inflows positive
        xirr_flows = [(actual_start_date, -initial_investment)]
        ...
        for action_date, action_type, row in actions:
            ...
            if action_type == "right":
                ...
                xirr_flows.append((action_date, -cost))
                ...
                if cash_pct > 0:
                    ...
                    xirr_flows.append((action_date, cash_rs))
        ...
        xirr_flows.append((latest_date, market_value))
        ...
        # ── Company name lookup ───────────────────────────────────────────────
        company_name = get_company_name(symbol, data_dir)
        ...
        return {
            "symbol": symbol.upper(),
            "company_name": company_name,           # ← NEW: right after symbol
            ...
            "multiple": round(todays_value / total_invested, 2),
        }
...
def main():
    ...
    if args.mode == "single" and args.investment == DEFAULT_INVESTMENT:
    ...
    if args.mode in ["multiple", "all"] and args.years and not args.min_years and sys.stdin.isatty():
    ...
    for symbol in symbols:
        ...
        if args.mode in ["multiple", "all"]:
            name_display = result.get("company_name", "") or result["symbol"]
            print(f"  ✓ {result['symbol']:<8} | {name_display:<40} | CAGR: {result['cagr_pct']:>7.2f}% | Years: {result['years']:>6.2f} | "
    ...
    if args.mode in ["multiple", "all"] and results:
        ...
        print(f"\n  Top 10 by CAGR:")
        print(f"  {'Rank':<6} {'Symbol':<10} {'Company':<35} {'CAGR %':>10} {'Years':>8} {'Total Value':>15} {'Invested':>15} {'Multiple':>10}")
        print(f"  {'-'*101}")
        for i, r in enumerate(results_sorted[:10], 1):
            name_display = r.get("company_name", "") or ""
            print(f"  {i:<6} {r['symbol']:<10} {name_display:<35} {r['cagr_pct']:>10.2f}% {r['years']:>8.2f} "
                  f"{r['todays_value']:>15,.0f} {r['total_invested']:>15,.0f} {r.get('multiple', 0):>9.2f}x")
