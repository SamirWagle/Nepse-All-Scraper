...
--------------
    ...
    python nepse_cagr_modified.py --mode all --years 13 --output-csv custom_name.csv
"""
...
def main():
    ...
    parser.add_argument("--output-csv", help="Save results to CSV file (for multiple/all mode). Default: auto-saves to Research folder")
    parser.add_argument("--min-years", type=float, help="Minimum years of data required (excludes stocks with less data)")
    parser.add_argument("--no-filter", action="store_true", help="Don't filter stocks by minimum years (include all)")
    ...
    if args.mode in ["multiple", "all"] and args.years and not args.min_years and not args.no_filter:
        args.min_years = args.years
        print(f"\n  ✓ Auto-filtering: excluding stocks with less than {args.years} years of data")
    ...
    # ── Auto-generate CSV filename for multiple/all mode ────────────────
    if args.mode in ["multiple", "all"] and not args.output_csv:
        # Auto-save to Research folder with timestamped filename
        research_folder = Path(__file__).parent.parent / "Research"
        research_folder.mkdir(exist_ok=True)
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        years_label = f"{args.years:.0f}yr" if args.years else "custom"
        filename = f"nepse_cagr_{years_label}_{timestamp}.csv"
        args.output_csv = research_folder / filename
        print(f"  ✓ Results will be saved to: {args.output_csv.name}")
    ...
    for symbol in symbols:
        ...
        if args.min_years and result.get("years", 0) < args.min_years:
            if args.mode in ["multiple", "all"]:
...
if __name__ == "__main__":
    main()
ENDOFFILEcat > "nepse_cagr_modified_13 yrs_new.py" << 'ENDOFFILE'
"""
NEPSE Stock CAGR Calculator
============================
Uses data from: https://github.com/SamirWagle/Nepse-All-Scraper
